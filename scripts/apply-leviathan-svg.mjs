/**
 * Apply scripts/svg-leviathan.mjs into myman_mascot.js
 * Inserts after aether block / before integrated accessories.
 */
import fs from 'fs';
...
const mod = await import(pathToFileURL('scripts/svg-leviathan.mjs').href + '?t=' + Date.now());
let svg = String(mod.leviathanSvg).replace(/\r\n/g, '\n').replace(/\n/g, nl);
if (!svg.endsWith(nl)) svg += nl;

let src = fs.readFileSync(path, 'utf8');

const startMarker = '<!-- LEVIATHAN CHARACTER - All Life Stages';
const accessoriesRe = /[ \t]*<!-- Integrated accessories \(anchor-local art, positioned by layoutMascotAccessory\) -->/;

const endM = src.match(accessoriesRe);
...
if (existingStart >= 0 && existingStart < end) {
  ...
  console.log('replaced existing leviathan block');
} else {
  src = src.slice(0, end) + svg + src.slice(end);
  console.log('inserted new leviathan block');
}
...
for (const s of stages) {
  const id = `tm-mascot-${s}-leviathan`;
...
console.log('leviathan sprites applied ok');
